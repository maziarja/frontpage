...
- All server-side data fetching for Server Components lives in `app/_queries/` — one file per domain (e.g. `app/_queries/feed.ts`, `app/_queries/dashboard.ts`). Query functions accept primitive params (userId, feedId, etc.) and return fully typed data including derived UI flags (e.g. `showRetry`, `hasMore`). Pages and layouts must not contain Prisma queries or business logic — they resolve auth, call a query function, check for null (notFound()), and render.
